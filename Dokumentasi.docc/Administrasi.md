...
- Daftar transaksi secara otomatis menghapus data dari `CoreData` saat dihapus dari `CollectionView`.
    - **Hapus Data:** ``TransaksiView/deleteSelectedItems(_:)`` → simpan *snapshot* → hapus dari Core Data dan CollectionView → registrasi undoManager dengan *snaphsot* melalui parameter di ``TransaksiView/undoHapus(_:)``.
