...
class NilaiKelas: NSViewController {
    ...
    override func viewWillAppear() {
        ...
        TableType.fromString(namaKelas) { [weak self] kelas in
            guard let s = self,
                  let kelasModel = s.kelasAktif
                  ? s.viewModel.kelasData[kelas]
                  : s.viewModel.arsipKelasData[kelas]
            else { return }

            s.configureSemesterPopUp(kelasModel)
        }

        kelasLabel.stringValue = tahunAjaran.isEmpty
            ? namaKelas : namaKelas + "・" + tahunAjaran
    ...
    @IBAction func newWindow(_ sender: Any) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
            ...
            AppDelegate.shared.openedKelasWindows[namaKelas] = window
        }
    }
    ...
    @objc private func semesterSelectionChanged() {
        TableType.fromString(namaKelas) { [weak self] kelas in
            guard let s = self, let kelasModel = s.kelasAktif ? s.viewModel.kelasData[kelas] : s.viewModel.arsipKelasData[kelas],
                  let selectedSemester = s.semesterPopUp.titleOfSelectedItem
            else { return }
            ...
            Task(priority: .background) {
                ...
                s.filteredData = kelasModel.filter { $0.semester == semester }
                ...
                // Jalankan dua operasi async secara paralel
                async let studentSummaries = s.createCustomSummaries(forKelas: kelasModel, semester: semester)
                async let mapelSummaries = s.calculateMapelSummaries(forKelas: kelasModel, semester: semester)

                // Tunggu hasilnya
                let (data, mapelData) = await (studentSummaries, mapelSummaries)
                s.data = data
                s.mapelData = mapelData
    ...
    private func configureSemesterPopUp(_ data: [KelasModels]) {
    ...
    func tableView(_ tableView: NSTableView, rowActionsForRow row: Int, edge: NSTableView.RowActionEdge) -> [NSTableViewRowAction] {
        ...
        if edge == .leading {
            ...
            let copy = NSTableViewRowAction(style: .regular, title: "Salin Beserta Kolom") { [weak self] _, _ in
                ...
                attributedString.append(secondaryAttributedString)
                textField.attributedStringValue = attributedString
                workItemRight = DispatchWorkItem {
                    NSAnimationContext.runAnimationGroup({ context in
                        context.allowsImplicitAnimation = true
                        context.duration = 0.3
                        cell.alphaValue = 0
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.5, execute: workItemRight!)
            }
