...
class NilaiKelas: NSViewController {
    ...
    @IBAction func newWindow(_ sender: Any) {
        ...
        nilaiSiswaVC.jumlahnilai = self.jumlahnilai
        nilaiSiswaVC.namaKelas = self.namaKelas
        nilaiSiswaVC.kelasModel = self.kelasModel
...
extension NilaiKelas: NSTableViewDataSource {
    func numberOfRows(in tableView: NSTableView) -> Int {
        return 1 + data.count + 1 + mapelData.count // Headers + student data + subject header + subject data
    }
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        guard tableColumn?.identifier == NSUserInterfaceItemIdentifier("siswa"),
              let cell = tableview.makeView(withIdentifier: NSUserInterfaceItemIdentifier("combinedCell"), owner: self) as? NSTableCellView else {
            return nil
        }
