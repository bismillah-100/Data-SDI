...
extension SiswaViewController {
    func handleUndoActionGrouped(id: Int64, groupIndex: Int? = nil, rowInSection: Int? = nil, columnIndex: Int) {
        ...
        if let groupIndex = groupIndex, let rowInSection = rowInSection {
    ...
    @objc func handleUndoActionNotification(_ notification: Notification) {
        guard let userInfo = notification.userInfo,
              ...
              let columnIdentifier = userInfo["columnIdentifier"] as? String else {
            return
        }
    ...
    func updateDataInBackground(selectedRowIndexes: IndexSet) {
        ...
        if currentTableViewMode == .plain {
            ...
            self.view.window?.beginSheet(progressWindowController.window!)
        ...
        ///* Logika untuk mode grouped
            ...
            for (index, siswa) in selectedSiswaList.reversed().enumerated() {
                ...
                if siswa.kelasSekarang != updatedSiswa.kelasSekarang {
                    let oldAbsoluteVisualIndex = self.viewModel.getAbsoluteRowIndex(groupIndex: groupIndex, rowIndex: rowIndex) // Hitung SEBELUM remove/insert berikutnya
                    
                    let newGroupIndex = self.getGroupIndex(forClassName: updatedSiswa.kelasSekarang) ?? groupIndex
                    let insertIndex = self.viewModel.groupedSiswa[newGroupIndex].insertionIndex(for: updatedSiswa, using: sortDescriptor)
                    
                    self.viewModel.removeGroupSiswa(groupIndex: groupIndex, index: rowIndex)
                    self.viewModel.insertGroupSiswa(updatedSiswa, groupIndex: newGroupIndex, index: insertIndex)
                    let newAbsoluteVisualIndex = self.viewModel.getAbsoluteRowIndex(groupIndex: newGroupIndex, rowIndex: insertIndex) // Hitung SETELAH remove/insert
                    ...
                    if self.isBerhentiHidden && updatedSiswa.status.lowercased() == "berhenti" || (!UserDefaults.standard.bool(forKey: "tampilkanSiswaLulus") && updatedSiswa.status.lowercased() == "lulus") {
                        siswaData.append((groupIndex, rowIndex, updatedSiswa))
                    }
                }
