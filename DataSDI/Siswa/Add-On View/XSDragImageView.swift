...
class XSDragImageView: NSImageView, NSDraggingSource {
    ...
    private var highlightWorItem: DispatchWorkItem!
    ...
    func deleteImage(_: Any?) {
        ...
        {
            // NSViewController dari superview ditemukan dan sudah bertipe PratinjauFoto.
    ...
    private func unhighlight() {
        if let previousImage = imageNow {
            highlightWorItem?.cancel()
            // Bungkus dalam dispatch work item
            let workItem = DispatchWorkItem { [weak self] in
                ...
                self?.updateAddDataVC()
            }
            highlightWorItem = workItem
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1, execute: highlightWorItem)
        }
    }

    private func updateAddDataVC() {
        // Misalkan ini adalah kode di dalam NSView Anda atau di mana pun Anda memiliki akses ke `self` (NSView)
    ...
    private func updateHighlight() {
        if let image = draggedImage {
            ...
            let workItem = DispatchWorkItem { [weak self] in
                ...
                // Buat NSImage baru dengan background transparan sebesar frame imageView
                let paddedImage = NSImage(size: targetSize, flipped: false) { _ in
                    // Hitung rasio dan posisi gambar agar berada di tengah
                    let imageSize = image.size
                    let aspectWidth = targetSize.width / imageSize.width
                    let aspectHeight = targetSize.height / imageSize.height
