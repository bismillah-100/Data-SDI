...
class DatabaseController {
    ...
    private init() {
        ...
        self.dbPath = dataSiswaFolderURL.appendingPathComponent("data.sqlite3").path
    ...
    public func getGuru(forYear year: String) -> [GuruModel] {
        ...
        do {
            for user in try db.prepare(query) {
                let bentukGurus: GuruModel = GuruModel()
    ...
    public func getSiswa(idValue: Int64) -> ModelSiswa {
        let bentukSiswa: ModelSiswa = ModelSiswa()
        do {
    ...
    public func bacaFotoSiswa(idValue: Int64) -> FotoSiswa {
        var fotoSiswa: FotoSiswa = FotoSiswa()
        do {
    ...
    func getSiswaIDForNamaSiswa(_ namaSiswa: String) -> Int64? {
        do {
            if let row = try db.pluck(siswa.filter(self.nama == namaSiswa)) {
            ...
            resultData = try await withThrowingTaskGroup(of: MonthlyData.self) { group in
                for year in sortedYears {
                    group.addTask { [weak self] in
                        guard let self = self else { return MonthlyData(year: year) }
                                ...
                                monthGroup.addTask {
                                    let filteredSiswa = siswaArray.filter {
                                        guard let tanggalDaftar = try? $0.get(self.tahundaftar),
                                              ...
                                              let monthDaftar = self.getMonthFromDate(dateString: tanggalDaftar) else {
                                            return false
                                        }
    ...
    func updateUndoStackKelas(idSiswa: Int64, kelas: String) async {
        ...
        case "Kelas 1":
            ...
            allKelasModels = ["Kelas 1": await getallKelas1()]
        case "Kelas 2":
            guard !SingletonData.table2dimuat else { return }
            allKelasModels = ["Kelas 2": await getallKelas2()]
        case "Kelas 3":
            guard !SingletonData.table3dimuat else { return }
            allKelasModels = ["Kelas 3": await getallKelas3()]
        case "Kelas 4":
            guard !SingletonData.table4dimuat else { return }
            allKelasModels = ["Kelas 4": await getallKelas4()]
        case "Kelas 5":
            guard !SingletonData.table5dimuat else { return }
            allKelasModels = ["Kelas 5": await getallKelas5()]
        case "Kelas 6":
            guard !SingletonData.table6dimuat else { return }
            allKelasModels = ["Kelas 6": await getallKelas6()]
        default:
            break
        }
        ...
        for (kelasSekarang, kelasModels) in allKelasModels {
            ...
            if !siswaData.isEmpty {
                ...
                let existingSiswaIDs = SingletonData.undoStack[kelasSekarang]?.flatMap { $0 }.map { $0.siswaID } ?? []
...
extension Connection {
    ...
    func getColumnDetails(in tableName: String) throws -> [(name: String, type: ColumnDefinition.Affinity, notNull: Bool, primaryKey: ColumnDefinition.PrimaryKey?)] {
        ...
        let columnsSchema = try self.schema.columnDefinitions(table: tableName)
