...
final class DynamicTable {
    ...
    func renameColumn(_ namaTabel: String, kolomLama: String, kolomBaru: String) async throws {
        guard let db = db, let index = SingletonData.columns.firstIndex(where: {$0.name == kolomLama}) else {
            return
        }
            ...
            let insert = mainTable.insert(
                newData.map { key, value in
                    ...
                    case let stringValue as String:
                        return Expression<String>(key) <- stringValue
                    case let intValue as Int64:
                        return Expression<Int64>(key) <- intValue
                    case let dataValue as Data:
                        return Expression<Data>(key) <- dataValue
                    default:
                        return nil  // Atau Anda bisa menambahkan penanganan kesalahan di sini
                    }
