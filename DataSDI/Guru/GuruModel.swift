...
class GuruModel: Comparable {
    ...
    func copy() -> GuruModel {
        ...
        newCopy.idGuru = self.idGuru
        newCopy.namaGuru = self.namaGuru
        newCopy.alamatGuru = self.alamatGuru
        newCopy.tahunaktif = self.tahunaktif
        newCopy.mapel = self.mapel
        newCopy.struktural = self.struktural
        return newCopy
    }
}
...
extension Array where Element == MapelModel {
    ...
    func insertionIndex(for element: Element, using sortDescriptor: NSSortDescriptor) -> Index {
        return self.firstIndex { item in
            ...
            case "Mapel":
                ...
                return sortDescriptor.ascending ? item.namaMapel >= element.namaMapel : item.namaMapel <= element.namaMapel
            default:
