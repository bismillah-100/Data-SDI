...
extension TugasMapelVC {
    ...
    func buatMenuItem() -> NSMenu {
        ...
        // MARK: - Item Menu `Hanya Tampilkan Tugas Aktif`

        let status = NSMenuItem(title: "Tampilkan Tugas Selesai", action: #selector(filterTugas(_:)), keyEquivalent: "")
        status.identifier = NSUserInterfaceItemIdentifier("filterTugas") // Memberikan identifier unik.
        status.target = self
        status.state = viewModel.filterTugas == nil ? .on : .off
        menu.addItem(status)
        ...
        guard clickedRow >= 0 else {
            for i in menu.items {
                ...
                if i.identifier?.rawValue == "add" ||
                    ...
                    i.identifier?.rawValue == "filterTugas"
                {
                    i.isHidden = false
                }
