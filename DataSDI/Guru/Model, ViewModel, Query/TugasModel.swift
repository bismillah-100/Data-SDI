...
enum PenugasanGuruMapelKelasColumns {
    ...
    static let id: SQLite.Expression<Int64> = .init("id_penugasan")
    /// Kolom 'id_guru' pada tabel `penugasan_guru_mapel_kelas`, merujuk ke `guru.id`.
    static let idGuru: SQLite.Expression<Int64> = .init("id_guru")
    /// Kolom "id_jabatan' pada tabel `penugasan_guru_mapel_kelas`, merujuk ke `jabatan.id`.
    static let idJabatan: SQLite.Expression<Int64> = .init("id_jabatan")
    ...
    static let tanggalMulaiEfektif: SQLite.Expression<String> = .init("tanggal_mulai_efektif")
    /// Kolom 'tanggal_selesai_efektif' pada tabel `penugasan_guru_mapel_kelas`, bisa bernilai null.
    static let tanggalSelesaiEfektif: SQLite.Expression<String?> = .init("tanggal_selesai_efektif")
    /// Kolom 'status_penugasan' pada tabel `penugasan_guru_mapel_kelas`, contoh: "Aktif".
    static let statusPenugasan: SQLite.Expression<Int> = .init("status_penugasan")
}
