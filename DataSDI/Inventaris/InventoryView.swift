...
//            }
    ...
    func saveTableInfo() {
        // Simpan urutan kolom
        let columnOrder = tableView.tableColumns.map { $0.identifier.rawValue }
    ...
    @IBAction func increaseSize(_ sender: Any?) {
        ReusableFunc.increaseSize(self.tableView)
        saveRowHeight()
    }
    
    /// Lihat: ``DataSDI/ReusableFunc/decreaseSize(_:)``
    @IBAction func decreaseSize(_ sender: Any?) {
        ReusableFunc.decreaseSize(self.tableView)
        saveRowHeight()
    }
...
extension InventoryView: NSTableViewDataSource {
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        guard let cellView = tableView.makeView(withIdentifier: NSUserInterfaceItemIdentifier(rawValue: cellIdentifier), owner: self) as? NSTableCellView,
              let cellImage = tableView.makeView(withIdentifier: NSUserInterfaceItemIdentifier(imageCellIdentifier), owner: self) as? NSTableCellView else {
            return nil
        }
...
extension InventoryView {
    ...
    func handleInsertNewRows(at startRow: Int, fileURLs: [URL], tableView: NSTableView) async -> Bool {
        ...
        for fileURL in fileURLs {
            ...
            guard let image = NSImage(contentsOf: fileURL), // Coba muat gambar dari URL.
                  let imageData = image.tiffRepresentation else { // Dapatkan representasi TIFF dari gambar.
            ...
            let (inserted, id) = await self.handleInsertNewRow(at: currentRow, withImageData: imageData, tableView: tableView, fileName: fileName)
    ...
    func findRowIndex(forId id: Int64) -> Int? {
        return data.firstIndex { ($0["id"] as? Int64) == id }
    }
}
...
extension InventoryView {
    ...
    func redoAddColumn(columnName: String) {
        ...
        tableView(self.tableView, sortDescriptorsDidChange: self.tableView.sortDescriptors)
        setupColumnMenu()
        updateUndoRedo()
        setupDescriptor()
    }
    ...
    @IBAction func addColumnButtonClicked(_ sender: Any) {
        ...
        alert.beginSheetModal(for: self.view.window!) { [self] (response) in
    ...
    func editNamaKolom(_ kolomLama: String, kolomBaru: String) {
        ...
        self.myUndoManager.registerUndo(withTarget: self) { [weak self] handler in
    ...
    func undoEditNamaKolom(kolomLama: String, kolomBaru: String, previousValues: [Int64: Any]) {
        Task(priority: .background) { [weak self] in
            guard await ((try? DynamicTable.shared.renameColumn("main_table", kolomLama: kolomBaru, kolomBaru: kolomLama)) != nil), let self = self else { return }
...
extension InventoryView {
    ...
    func undoReplaceImage(_ id: Int64, imageData: Data) {
        guard let row = self.findRowIndex(forId: id) else {
            return
        }
        let newImage = self.data[row]["Foto"] as? Data
        self.data[row]["Foto"] = imageData
        Task {
    ...
    func redoReplaceImage(_ id: Int64, imageData: Data) {
        ...
        let oldImage = self.data[row]["Foto"] as? Data
        self.data[row]["Foto"] = imageData
...
extension InventoryView: NSMenuDelegate {
    ...
    @objc func edit(_ sender: Any) {
        ...
        tableView.selectedRowIndexes.reversed().forEach({ row in
        ...
        tableView.tableColumns.forEach({ column in
            guard column.identifier.rawValue != "id",
                  column.identifier.rawValue != "Tanggal Dibuat",
                  column.identifier.rawValue != "Foto"
            else {return} // Lewati kolom-kolom yang tidak boleh diedit.
            ...
            Task {
                ...
                for updatedData in updatedRows {
                    ...
                    guard let idValue = updatedData["id"] as? Int64,
                          let newValue = updatedData[selectedColumn] as? String else {
...
extension InventoryView: NSFilePromiseProviderDelegate {
    ...
    func filePromiseProvider(_ filePromiseProvider: NSFilePromiseProvider,
        ...
        guard let userInfoDict = filePromiseProvider.userInfo as? [String: Any],
              let row = userInfoDict[FilePromiseProvider.UserInfoKeys.rowNumberKey] as? Int else {
...
extension InventoryView: OverlayEditorManagerDataSource {
    ...
    func overlayEditorManager(_ manager: OverlayEditorManager, originalColumnWidthForCellAtRow row: Int, column: Int, in tableView: NSTableView) -> CGFloat {
        ...
        return tableView.tableColumns[column].width // Sesuaikan jika perlu
    }
