            ...
            do {
                ...
                for result in results {
                    guard let tanggal = result["tanggal"] as? Date,
                          let total = result["yearTotal"] as? Double else {
                        continue
                    }
                ...
                if filterJenis == "Jumlah Saldo" {
                    ...
                    years = sortedData.map { $0.0 }
                    cumulativeSaldo = sortedData.map { $0.1 }
...
class CustomYAxisValueFormatter: AxisValueFormatter {
    ...
    func stringForValue(_ value: Double, axis: AxisBase?) -> String {
        if value >= 1_000_000_000 {
            return "\(String(format: "%.1f M", value / 1_000_000_000))"
        } else if value >= 1_000_000 {
            return "\(String(format: "%.1f Jt", value / 1_000_000))"
        } else if value >= 1_000 {
            return "\(String(format: "%.1f Rb", value / 1_000))"
        } else {
            return "\(Int(value))"
        }
    }
