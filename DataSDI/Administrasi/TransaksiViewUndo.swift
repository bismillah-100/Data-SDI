...
extension TransaksiView {
    ...
    func undoHapus(_ snapshots: [EntitySnapshot]) {
        ...
        self.myUndoManager.registerUndo(withTarget: self) { [weak self] targetSelf in
            guard let self = self else { return } // Pastikan self masih ada.
    ...
    func undoEdit(_ entityBackup: [EntitySnapshot]) {
        ...
        entityBackup.forEach { snapshotToRestore in
                ...
                for (section, jenisTransaksi) in sortedSectionKeys.enumerated() {
                    ...
                    for (itemIndex, entity) in items.enumerated().reversed() {
                        if let entityId = entity.id, ids.contains(entityId) {
                            ...
                            if let oldData = prevData.first(where: {$0.id == entityId}) {
                                oldGroupKey = getGroupKey(for: oldData) ?? "Lainnya"
                            } else {
                                oldGroupKey = "Lainnya"
                            }
    ...
    func getEntityGroupKey(for snapshot: Entity) -> String? {
        ...
        case "acara":
            ...
            return snapshot.acara ?? "Tanpa Acara"
        case "kategori":
            ...
            return snapshot.kategori ?? "Tanpa Kategori"
        case "keperluan":
            ...
            return snapshot.keperluan ?? "Tanpa Keperluan"
        default:
    ...
    func getSortingCriteria(for option: String) -> [String] {
        ...
        case "terbaru", "terlama":
            ...
            return [option, "keperluan", "acara", "kategori", "jumlah"]
        case "kategori":
        ...
        case "keperluan":
            ...
            return [option, "acara", "kategori", "jumlah", "terbaru"]
        case "jumlah":
            ...
            return [option, "acara", "kategori", "transaksi", "terbaru"]
        case "transaksi":
            ...
            return [option, "acara", "keperluan", "jumlah", "terbaru"]
        case "bertanda":
