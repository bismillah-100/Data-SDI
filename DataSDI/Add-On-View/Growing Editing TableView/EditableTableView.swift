...
class EditableTableView: NSTableView {
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        let clickedColumn = self.column(at: localPoint)
        ...
        let isRowCurrentlySelected = self.selectedRowIndexes.contains(clickedRow)

        if isRowCurrentlySelected {
            // Kondisi 2 & 3: Apakah klik mengenai NSTextField di dalam sel dan ada teksnya?
            if let cellView = self.view(atColumn: clickedColumn, row: clickedRow, makeIfNecessary: false) as? NSTableCellView,
...
//        }
        ...
        if (event.keyCode == enterKeyCode || event.keyCode == keypadEnterKeyCode),
           selectedRow >= 0 {
            ...
            self.editAction?(selectedRow, defaultEditColumn ?? 0)
